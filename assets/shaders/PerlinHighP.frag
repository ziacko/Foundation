...
void main()
{
	...
	vec2 q2 = (inBlock.uv - uvOffset) * uvScale;
	q *= uvScale;
	vec2 p = -1.0 + 2.0 * q;
	vec2 p2 = -1.0 + 2.0 * q2;
	vec2 qQ;
	vec2 r;
	float color = Pattern2(p, qQ, r, float(totalTime * 0.05)) * colorBias;
	//just do it again?
	float color2 = Pattern2(p, qQ, r, float(totalTime * 0.05)) * colorBias;
	//assuming the correct data is being passed into the rendertarget
	//uint output = packHalf2x16(vec2(color, color2));

	//color = color - int(color);

	outColor = vec2(color, color2);// output;	
}