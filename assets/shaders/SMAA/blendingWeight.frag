#version 450
#define SMAA_DEPTH_THRESHOLD (0.1 * 1.0f)
#define SMAA_AREATEX_MAX_DISTANCE 160
#define SMAA_AREATEX_MAX_DISTANCE_DIAG 200
...
layout(binding = 0) uniform sampler2D edgesTexture;
...
vec2 SMAASearchDiag1(sampler2D edgesTex, vec2 texcoord, vec2 dir, out vec2 e) 
{
	...
	vec3 t = vec3(deltaResolution.xy, 1.0);
	while (coord.z < float(maxSearchStepsDiag - 1) && coord.w > 0.9) 
	{
...
{
	vec4 coord = vec4(texcoord, -1.0, 1.0);
	coord.x += 0.25 * deltaResolution.x; // See @SearchDiag2Optimization
	vec3 t = vec3(deltaResolution.xy, 1.0);
	while (coord.z < float(maxSearchStepsDiag - 1) && coord.w > 0.9) 
	{
		coord.xyz = fma(t, vec3(dir, 1.0), coord.xyz);
...
vec2 SMAACalculateDiagWeights(sampler2D edgesTex, sampler2D areaTex, vec2 texcoord, vec2 e, vec4 subsampleIndices) 
{
	...
	if (d.x + d.y > 2.0) { // d.x + d.y + 1 > 3
		...
		vec4 coords = fma(vec4(-d.x + 0.25, d.x, d.y, -d.y - 0.25), deltaResolution.xyxy, texcoord.xyxy);
		vec4 c;
	...
	if (d.x + d.y > 2.0) 
	{
		 // d.x + d.y + 1 > 3
		// Fetch the crossing edges:
		vec4 coords = fma(vec4(-d.x, -d.x, d.y, d.y), deltaResolution.xyxy, texcoord.xyxy);
		vec4 c;
...
float SMAASearchXLeft(sampler2D edgesTex, sampler2D searchTex, vec2 texcoord, float end) 
{
	...
	{ // Or is there a crossing edge that breaks the line?
		...
		texcoord = fma(-vec2(2.0, 0.0), deltaResolution.xy, texcoord);
	}

	float offset = fma(-(255.0 / 127.0), SMAASearchLength(searchTex, e, 0.0), 3.25);
	return fma(deltaResolution.x, offset, texcoord.x);
...
float SMAASearchYUp(sampler2D edgesTex, sampler2D searchTex, vec2 texcoord, float end)
{
	...
	{
		e = textureLod(edgesTex, texcoord, 0.0).rg;
		texcoord = fma(-vec2(0.0, 2.0), deltaResolution.xy, texcoord);
	}
	float offset = fma(-(255.0 / 127.0), SMAASearchLength(searchTex, e.gr, 0.0), 3.25);
	return fma(deltaResolution.y, offset, texcoord.y);
}
...
void SMAADetectHorizontalCornerPattern(sampler2D edgesTex, inout vec2 weights, vec4 texcoord, vec2 d)
{
	...
	vec2 rounding = (1.0 - float(cornerRounding) / 100.0) * leftRight;
