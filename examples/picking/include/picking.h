...
public:
	...
	picking(const char* windowName = "Ziyad Barakat's Portfolio(picking)",
		camera_t* camera3D = new camera_t(glm::vec2(1280, 720), 0.1f, camera_t::projection_t::perspective),
		model_t* model = new model_t("../../resources/models/Cube.fbx", false, true),
	...
	void Initialize() override
	{
		...
		geometryBuffer->AddAttachment(new frameBuffer::attachment_t("stencil", stencilDesc));
		...
		renderGrid = new grid(glm::vec2(2));
	}
...
protected:
	...
	virtual void GeometryPass() override
	{
		...
		if (!testModel->meshes[0].textures.empty())
		{
		...
		glDrawElements(GL_TRIANGLES, (GLsizei)renderGrid->indices.size(), GL_UNSIGNED_INT, 0);
	...
	virtual void StencilPass() override
	{
		geometryBuffer->Bind();
		//enable stencils, 
	...
	void RayCast()
	{
		...
		raycast::ray_t testRay;
		raycast::RayFromMouse(*sceneCamera, defaultPayload.data.mousePosition, *renderGrid, transform_t());
	...
	{
		defaultVertexBuffer = new vertexBuffer_t(defaultPayload.data.resolution / glm::vec2(10));
	}
};
